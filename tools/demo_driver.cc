...
int main( int argc, char* argv[] ) try
{
	...
	std::unique_ptr<artdaq::CommandableFragmentGenerator> const gen( artdaq::makeCommandableFragmentGenerator(
	    fragment_receiver_pset.get<std::string>( "generator" ), fragment_receiver_pset ) );
	...
	if ( !want_artapp ) { os << event_builder_pset.get<int>( "events_expected_in_SimpleMemoryReader" ); }
	...
	artdaq::EventStore::ART_CMDLINE_FCN* es_fcn( want_artapp ? &artapp : &artdaq::SimpleMemoryReaderApp );

	artdaq::EventStore store( event_builder_pset, event_builder_pset.get<size_t>( "expected_fragments_per_event" ),
	                          complete_pset.get<artdaq::EventStore::run_id_t>( "run_number" ), es_argc, es_argv,
	                          es_fcn );
	...
	while ( events_to_generate >= 0 && gen->getNext( frags ) )
	{
		for ( auto& val : frags )
		{
			std::cout << "Fragment: Seq ID: " << val->sequenceID() << ", Frag ID: " << val->fragmentID()
			          << ", total size in bytes: " << val->size() * sizeof( artdaq::RawDataType ) << std::endl;
			...
			if ( events_to_generate != 0 && event_count > events_to_generate ) gen.get()->StopCmd( timeout, timestamp );
