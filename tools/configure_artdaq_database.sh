...
function configure_artdaq_database()
{
    ...
    local this_database_version=${arg_database_version:-$(source $this_setup_script > /dev/null 2>&1 && \
    ...
    local this_database_upsrepo=$(source $this_setup_script > /dev/null 2>&1 && \
         ups list -a artdaq_database -q $this_database_quals |grep -B1 "Version=" |grep "DATABASE=" |cut -d "=" -f 2 | head -1 || echo $error_val)
    ...
    if [[ ! -d $this_extra_products ]]; then
      ret_msg="Error: \"$this_extra_products\" does not exist."
      return 9
    fi
...
function load_configs()
{
  ...
  [[ ! -f $DAQINTERFACE_USER_SOURCEFILE.orig ]] && cp -f $DAQINTERFACE_USER_SOURCEFILE{,.orig}

  cp -f $DAQINTERFACE_USER_SOURCEFILE{.bak,}
  local this_database_setup_cmd=$(ups active |grep artdaq_database|awk '{printf "setup %s %s %s %s",  $1,$2,$5,$6}')
...
function stop_daqinterface_if_running(){
    ...
    "running:call_stop:ready,stopped:def_transition_timeout"
    "ready:call_terminate:stopped:def_transition_timeout"
    "stopped:call_kill_daq_unterface:terminated:def_transition_timeout")
